...
public class Client {
  ...
  public boolean sendMessage(String messageBody) {
    if (this.communicator == null) {
      throw new ExceptionInInitializerError(
          "No server information set. Call Client.setServer method.");
    }
  ...
  /**
   * Calls the server and returns messages that have been sent since we last called Requires server
   * is set
   *
  ...
  /**
   * Called on Client initialization only, loads most recent n messages from server, where n is
   * configured in server repo Requires server is set
   *
   * @return - List of Messages vended by server
   */
  ...
  /**
   * Prompts the client for information then connects to a running server, if possible Also begins
   * GetMessages routine to consistently pull for new messages from the server
   *
   * @param args
   */
    ...
    while (true) {
      ...
      if (message.equals("'Exit'")) {
        ...
        if (!sentMessage) {
          ...
          for (int i = 0; i < 5; i++) {
            System.err.println(
                String.format("Message didn't send. Will attempt to send %d more times", 5 - i));
            try {
              Thread.sleep(2000);
